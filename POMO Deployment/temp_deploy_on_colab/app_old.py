...
def next_forecast():
    if request.method == 'POST':
        ...
        models_list = ["tempC_avg_0C__pro.pkl", "Relative_humidity_avg__pro.pkl", "windspeedKmph_avg_Km_h__pro.pkl", "pressureMB_avg_pro.pkl",  "precipMM_avg_mm__pro.pkl", "weatherDesc_pro.pkl", "Sunshine_Hours_pro.pkl", "__soil_moisure_pro.pkl"]
        ...
        col_list = ['observation','tempC_avg(0C)', 'Relative humidity_avg(%)', 'windspeedKmph_avg(Km/h)', 'pressureMB_avg', 'precipMM_avg(mm)', 'weatherDesc', 'Sunshine Hours', '%_soil_moisure']
        #rename all column names
        df3.columns = col_list 
        # convert columns to int64 dtype 
        df3 = df3.astype({"tempC_avg(0C)": int, "Relative humidity_avg(%)": int, "windspeedKmph_avg(Km/h)": int, "pressureMB_avg": int, "weatherDesc": int, "%_soil_moisure": int})
